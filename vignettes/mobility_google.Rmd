...
sah_filtered <- sah %>% filter(county == 'All')
  
# join sah and google data
df_joined <- inner_join(mob_data_filtered, sah_filtered, by=c('sub_region_1' = c('state')))

# mobility change in NY County by type
df_joined %>% 
  filter(sub_region_1 == "New York" & sub_region_2 == "New York County" & date < order_date) %>%

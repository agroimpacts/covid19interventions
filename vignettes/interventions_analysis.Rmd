...
state_before_df <- inner_join(state_before,df3, by = c("SAH_State_Date" = "date",
                                    "admin1" = "state1")) %>% mutate(case_pop = cases / pop)
...
#State After
state_after <- df1
state_after$SAH_State_Date = state_before$SAH_State_Date + 28
...
county_intervention_cases <- final_df4
usethis::use_data(county_intervention_cases, overwrite = TRUE)
...
p <- ggplot(stateaft, aes(map_id = admin1)) + 
  ...
  theme(legend.position = "bottom", 
        panel.background = element_blank())+ theme(legend.key.width = unit(5, "cm")) + ggtitle("Normalized State Case Occurence 4 Weeks After SAH Intervention")
p

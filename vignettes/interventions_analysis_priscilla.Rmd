...
```{r}
sah_count <- sah_join %>% group_by(state, order_type, politicalparty) %>% 
  summarize(count = n_distinct(order_date))
```
...
p2 <- ggplot(state_tb_fit, aes(x=date, y=cases_log)) + geom_point() +
  ...
  labs(title = "COVID-19 Cases in Natural Log Scale",
     subtitle = firstup(statename[[1]]),
     x = "Date", y = "Cumulative Cases (log scale)") +
  theme(axis.text.x = element_text(angle = 90)) +
  theme(legend.title = element_blank())
p2
```
